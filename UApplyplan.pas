...
begin
  try
    ...
    if (GroupBoxspecik.Visible) then
      CheckBoxpressspecifik.checked := Prefs.PressSpecific OR Prefs.EnablePressSpecificSelection;
    ...
    begin
      DataM1.Query1.SQL.Clear;
      DataM1.Query1.SQL.add('Select DefaultApprove,DefaultHardProofID,DefaultProofID,PageFormatID from publicationnames');
      DataM1.Query1.SQL.add('Where publicationid = ' + IntToStr(plateframespublicationid));
      DataM1.Query1.open;
      IF not DataM1.Query1.eof then
      begin
        ...
        DefaultPageFormatID := DataM1.Query1.fields[3].AsInteger;
  ...
  begin
    ...
    try
      ...
      DataM1.Query1.SQL.add('SELECT ISNULL(DeviceID,0),ISNULL(RipSetup,''''),ISNULL(Hold,1) from PublicationTemplates (NOLOCK)');
      DataM1.Query1.SQL.add('WHERE PressID = '+inttostr(plateframespressid));
      DataM1.Query1.SQL.add('AND PublicationID = '+inttostr(plateframespublicationid));
      ...
      if (not DataM1.Query1.eof) then
      begin
        if DataM1.Query1.Fields[0].AsInteger > 0 then          // DeviceID
        begin
          DefaultDevID := DataM1.Query1.fields[0].AsInteger;  // OK
        end;
        if DataM1.Query1.Fields[1].AsString <> '' then          // RipSetup
        begin
          RipSetup := DataM1.Query1.fields[1].AsString;  // OK
        end;
        DefaultHold := DataM1.Query1.fields[2].asinteger;  // OK
      end;
  ...
  // NAN 201560804 - overrules publication defaults - always..
   ...
   if (Prefs.DefaultToApprovalRequired) then
    ...
    ComboBoxPageFormat.Items.Add('Default');
    DataM1.Query1.SQL.Clear;
    DataM1.Query1.SQL.Add('Select DISTINCT PageFormatName,PageFormatID from PageFormatNames Order by PageFormatID');
    DataM1.Query1.Open;
    while not DataM1.Query1.Eof do
    begin
      Inc(i);
      ComboBoxPageFormat.Items.Add(DataM1.Query1.fields[0].AsString);
      if (DefaultPageFormatID = DataM1.Query1.fields[1].AsInteger) then
        j := i;
      DataM1.Query1.Next;
    end;
    DataM1.Query1.Close;
    ComboBoxPageFormat.Itemindex := j;
end;
